#!/usr/bin/env bash
# ##
# ActiveMQ security group setup with AWS CloudFormation for Customer Self Service Portal - CuSSP
# ##

# Stack name of our network setup
STACK_NAME=CuSSP-MQ-security
...
validate() {
    aws cloudformation validate-template --template-body file://activemq-security.yml
}
...
create() {
    ...
    --template-body file://activemq-security.yml \
    --parameters \
...
# print usage
usage() {
    echo -e "\nActiveMQ security group setup with AWS CloudFormation for Customer Self Service Portal.\n"
    echo "Usage: $0 < \
[-p <profile>] \
[-t <stack name>] \
