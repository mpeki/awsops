...
init(){
    if [ -z ${BUCKET_STACK_NAME} ]; then BUCKET_STACK_NAME=DoDGame-s3-${TARGET_PROFILE}-bucket; fi
    if [ -z ${BUCKET_NAME} ]; then BUCKET_NAME=dodgame-${TARGET_PROFILE,,}; fi #must be lowercase
}

# validate template
validate() {
    aws cloudformation validate-template --template-body file://s3-dodgame-bucket.yml
}
...
create() {
    ...
    --template-body file://s3-dodgame-bucket.yml \
    --parameters \
