#!/usr/bin/env bash
# ##
# RDS subnet/security -group setup with AWS CloudFormation for Customer Self Service Portal - CuSSP
# ##
...
validate() {
    aws cloudformation validate-template --template-body file://rds-subnetgroup.yml
}
...
create() {
    ...
    --template-body file://rds-subnetgroup.yml \
    --parameters \
...
# print usage
usage() {
    echo -e "\nRDS subnet/security -group setup with AWS CloudFormation for Customer Self Service Portal.\n"
    echo "Usage: $0 < \
[-p <profile>] \
[-t <stack name>] \
